#Escribir un programa que gestione las facturas pendientes de cobro de una empresa. 
#Las facturas se almacenarán en un diccionario donde la clave de cada factura será el número de factura y el valor el coste de la factura. 
#El programa debe preguntar al usuario si quiere añadir una nueva factura, pagar una existente o terminar. 
#Si desea añadir una nueva factura se preguntará por el número de factura y su coste y se añadirá al diccionario. 
#Si se desea pagar una factura se preguntará por el número de factura y se eliminará del diccionario. 
#Después de cada operación el programa debe mostrar por pantalla la cantidad cobrada hasta el momento y la cantidad pendiente de cobro.

facturas={}
fin=False
cantidadcobrada=0
cantidadacobrar=0
while fin==False :
    operacion=str(input("¿Que desea hacer? (NUEVA FACTURA/PAGAR FACTURA/SALIR):"))
    while operacion.upper()!="NUEVA FACTURA" and operacion.upper()!="PAGAR FACTURA" and operacion.upper()!="SALIR" :
        print("ERROR, LA OPERACION QUE HAS SELECCIONADO NO ES CORRECTA, RESPONDA CORRECTAMENTE!")
        operacion=str(input("¿Que desea hacer? (NUEVA FACTURA/PAGAR FACTURA/SALIR):"))
    if operacion.upper()=="NUEVA FACTURA" :
        numerofactura=input("Introduzca el numero de factura:")
        costefactura=float(input("Introduzca el coste de la factura:"))
        facturas[numerofactura]=costefactura
        print(facturas)
        cantidadacobrar+=costefactura
    elif operacion.upper()=="PAGAR FACTURA" :
        numerofactura=input("Introduzca el numero de la factura que desea pagar:")
        costefactura=facturas.pop(numerofactura)
        print(facturas)
        cantidadcobrada+=costefactura
        cantidadacobrar-=costefactura
    else :
        fin=True
print("Importe recaudado: "+str(cantidadcobrada)+"€")
print("Deudas: "+str(cantidadacobrar)+"€")